...
function cadastrarGestor(idUsuario, nomeGestor, emailGestor, senhaGestor) {
    ...
    //  e na ordem de inserção dos dados.
    var instrucaoSql = `
        INSERT INTO gestor (fkUsuario, nomeGestor, emailGestor, senhaGestor) VALUES ('${idUsuario}', '${nomeGestor}', '${emailGestor}', '${senhaGestor}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
...
function autenticar(emailEmpresa, senha) {
    ...
    var instrucaoSql = `
        SELECT idUsuario,  emailEmpresa  FROM usuario WHERE emailEmpresa = '${emailEmpresa}' AND senha = '${senha}';
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
